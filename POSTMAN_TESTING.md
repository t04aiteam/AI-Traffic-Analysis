...
reference). It has **10 endpoints** for vehicle/plate detection, tracking, and
license-plate OCR.

> Two endpoints — `POST /predict/plates/multiframe` and `POST /predict/plates/video`
> — also require the **fusion sidecar** running on port `8100`
> (see [`fusion_svc/POSTMAN_TESTING.md`](fusion_svc/POSTMAN_TESTING.md)). The
> other 8 work without it.
...
| 14  | `POST /predict/plates/multiframe` | `files`=3 crops | (sidecar **stopped**) | **`503`**, `FusionUnavailable` |
| 15  | `POST /predict/plates/video` | `file`=1 mp4 | `engine=mflpr2` | `200`, **list** of per-track `{track_id,...,fast,ppocr}` |
...
- **JSON** (`/`, `/health`, `/config`, `/reset`, `/predict/image`, `/predict/frame`,
  ...
  response pane.
- **Image** (`/predict/batch`, `/predict/plates/batch` with 1 file): the
  **Preview** tab renders the annotated JPEG. Save via **⋯ / Save Response →
  Save to a file** (`.jpg`).
...
| `multiframe` / `video` | `engine` | `mflpr2`\|`eott` | fusion engine (default `mflpr2`); `eott` outputs binarized |
| `multiframe` / `video` | `scale` | int | upscale factor — applied by `mflpr2`, **ignored by `eott`** |
...
- `503 FusionUnavailable` → start the fusion sidecar on `8100` first.
